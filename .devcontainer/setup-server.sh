...
# 3. START MONGODB (ADDED THIS SECTION)
# MONGO_CONTAINER_NAME="agenta-mongo"
# echo "--- [PROVISIONING] Starting MongoDB... ---"
# if [ ! "$(docker ps -q -f name=$MONGO_CONTAINER_NAME)" ]; then
#     docker run -d #       --name $MONGO_CONTAINER_NAME #       --network $AGENT_NETWORK #       -v mongo-data:/data/db #       mongo:6
# fi
...
# -----------------------------
# 5. PUBLISH & MAKE PORT PUBLIC
# -----------------------------
echo "--- [PROVISIONING] Publishing Codespaces port $SERVER_PORT ---"
gh codespace ports publish $SERVER_PORT -c "$CODESPACE_NAME"
