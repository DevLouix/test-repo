...
PUBLIC_URL=$(gh codespace view --json forwardedPorts   | jq -r ".forwardedPorts[] | select(.localPort==$SERVER_PORT) | .url")

if [ -z "$PUBLIC_URL" ] || [ "$PUBLIC_URL" == "null" ]; then
  echo "--- [PROVISIONING] ERROR: Failed to get public URL. Ensure port $SERVER_PORT is configured correctly in devcontainer.json. ---"
  exit 1
fi
