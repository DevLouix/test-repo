...
if ! gh codespace ports visibility $SERVER_PORT:public; then
  echo "--- [PROVISIONING] WARNING: Failed to set port visibility to public. Proceeding with default settings. ---"
fi

PUBLIC_URL=$(gh codespace ports --json | jq -r   ".[] | select(.sourcePort==$SERVER_PORT) | .browseUrl")

if [ -z "$PUBLIC_URL" ]; then
  echo "--- [PROVISIONING] ERROR: Failed to get public URL. ---"
  exit 1
fi
