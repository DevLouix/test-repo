...
while [ -z "$PUBLIC_URL" ] || [ "$PUBLIC_URL" = "null" ]; do
    ...
    # Query Codespace port info
    PUBLIC_URL=$(gh codespace ports         -c "$CODESPACE_NAME"         --json sourcePort,browseUrl         -q "map(select(.sourcePort == $SERVER_PORT)) | .[0].browseUrl"         2>/dev/null || echo "")
...
echo "Public URL fetched: $PUBLIC_URL"
