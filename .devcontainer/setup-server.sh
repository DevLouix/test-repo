...
DOCKER_IMAGE_URL=ghcr.io/devlouix/agenta-server:latest
...
# This is the "hostname" for Redis inside the Docker network.
export REDIS_URL="redis://$REDIS_CONTAINER_NAME:6379"
echo "--- [PROVISIONING] Redis is running. Internal URL: $REDIS_URL ---"

# 3. AUTHENTICATE AND PULL YOUR APPLICATION IMAGE
echo "--- [PROVISIONING] Authenticating to GitHub Container Registry... ---"
...
sleep 15
echo "--- [PROVISIONING] Making port public and fetching URL... ---"
...
if [ -z "$PUBLIC_URL" ]; then echo "--- [PROVISIONING] ERROR: Failed to get public URL. ---" && exit 1; fi
